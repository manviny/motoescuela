...
gantt.directive('gantt', ['Gantt', 'dateFunctions', 'mouseOffset', 'debounce', 'keepScrollPos', '$filter', function (Gantt, df, mouseOffset, debounce, keepScrollPos, $filter) {
    return {
        restrict: "EA",
        replace: true,
...
$scope.saveData({ fn: function(apiURL) {
    ...
    _($scope.gantt.rows).forEach(function(fila) { 
        console.debug(fila.id, fila); 

        // (columna) recorre todas las horas de una fila
        var horas = [];
        _(fila.tasks).forEach(function(hora) { 
            horas.push({
                "id": fila.id + hora.id,  // tiene identidad compuesta para no colisionar al cambiar de columna
                "subject": hora.subject, 
                "from": ($filter('date')(hora.from, "yyyy-MM-dd H:00")),
                "to":($filter('date')(hora.to, "yyyy-MM-dd H:00")), 
                "color": hora.color, 
                "classes": hora.classes, 
                "data": hora.data
            })
        });

        // (fila) recorre todas las filas
        calendario.push({ "id": fila.id, "description": fila.description, "tasks": horas}) 
       
    }) // foreach 

    // Objeto Calendario  listo para guardar
    var objetoCalendario = {"id":fecha, "datos": calendario};
